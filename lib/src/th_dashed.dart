...
/// 虚线控件
class THDashed extends StatelessWidget {
  const THDashed({
    Key? key,
  ...
  Widget build(BuildContext context) {
    if (direction == Axis.horizontal) {
      return SizedBox(
        ...
        height: height,
        child: CustomPaint(
          painter: THDashedPainter(
              color: color, strokeWidth: height ?? 1, direction: direction),
        ),
      );
    } else {
      return SizedBox(
...
/// 绘制虚线自定义控件
class THDashedPainter extends CustomPainter {
  THDashedPainter(
      {this.color = Colors.black,
      this.strokeWidth = 1,
      this.gap = 2,
