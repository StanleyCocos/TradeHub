...
import 'package:trade_hub/src/util/platform_util.dart';
import 'package:trade_hub/src/util/version_util.dart';

import 'th_font.dart';
import 'th_font_loader.dart';
...
/// 文本控件
/// 设计原则：
/// 1.为了使用更方便，所以对系统组件进行的扩展，需兼容系统控件所有功能，不能让用户使用THesign时，因不能满足系统功能而弃用。
/// 2.非系统已有属性，尽量添加注释
///
...
class THText extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (forceVerticalCenter && kTextForceVerticalCenterEnable) {
      ...
      var textFont = font ?? Font(size: 16, lineHeight: 24);
      var fontSize = style?.fontSize ?? textFont.size;
      var height = style?.height ?? textFont.height;

      paddingConfig ??= THTextPaddingConfig.getDefaultConfig();
  ...
  /// 提取成方法，允许业务定义自己的THTextConfiguration
  THTextConfiguration? getConfiguration(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType<THTextConfiguration>();
  }
...
/// TextSpan的THesign扩展，将部分TextStyle中的参数扁平化。
class THTextSpan extends TextSpan {
  /// 构造参数，扩展参数释义可参考[THText]中字段注释
  THTextSpan({
...
/// 存储可以自定义THText居中算法数据的内部控件
class THTextConfiguration extends InheritedWidget {
  /// forceVerticalCenter=true时，内置padding配置
  final THTextPaddingConfig? paddingConfig;

  /// 全局字体,kTextNeedGlobalFontFamily=true时生效
  final FontFamily? globalFontFamily;

  const THTextConfiguration({Key? key, required Widget child, this.paddingConfig, this.globalFontFamily})
      : super(key: key, child: child);

  @override
  bool updateShouldNotify(covariant THTextConfiguration oldWidget) {
    return paddingConfig != oldWidget.paddingConfig;
  }
}

/// 通过Padding自定义THText居中算法
class THTextPaddingConfig {
  static THTextPaddingConfig? _defaultConfig;
  static final Map _cacheMap = {};

  /// 获取默认配置
  static THTextPaddingConfig getDefaultConfig() {
    _defaultConfig ??= THTextPaddingConfig();
