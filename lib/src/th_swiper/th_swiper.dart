...
class THSwiperPagination extends SwiperPlugin {
  const THSwiperPagination(
      {this.alignment,
      this.key,
      this.margin = const EdgeInsets.all(10.0),
      this.builder = THSwiperPagination.dots});

  /// 圆点样式
  static const SwiperPlugin dots = THSwiperDotsPagination();

  /// 圆角矩形 + 圆点样式 默认宽度20，高度6
  static const SwiperPlugin dotsBar =
      THSwiperDotsPagination(roundedRectangleWidth: 20);

  /// 数字样式
  static const SwiperPlugin fraction = THFractionPagination();

  /// 箭头样式
  static const SwiperPlugin controls = THSwiperArrowPagination();
...
class THSwiperDotsPagination extends SwiperPlugin {
  ...
  Widget build(BuildContext context, SwiperPluginConfig config) {
    if (config.itemCount > 20) {
      print('warning: The itemCount is too big, '
          'we suggest use THFractionPaginationBuilder');
    }
    ...
    var color = this.color ??
        (config.outer
            ? THColor.grayE7E7E7
            : THColor.white.withOpacity(0.55));
...
class THFractionPagination extends SwiperPlugin {
  // container宽度
  final double? width;
...
class THSwiperArrowPagination extends SwiperPlugin {
      ...
      Visibility(
        ...
        child: GestureDetector(
          child: CircleAvatar(
            radius: radius ?? 10.0,
            backgroundColor:
                backgroundColor ?? THColor.black66000000,
            child: backArrow ??
                const Icon(
